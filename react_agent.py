...
class Agent:
    def __init__(self, system=''):   #its a setup
        self.system=system       #stores system prompt
        self.messages=[]         #stores empty chat history
        if self.system:          # if present add system messages to history
            self.messages.append({
                'role':'system', 'content':system,
            })

    def __call__(self, message):    #handles user input
        self.messages.append({'role':'user', 'content': message})#user ip
        result=self.execute()    #stores response
        self.messages.append({'role':'assistant', 'content':result})   #returns reply
        return result
    
    def execute(self):      #talks to llm
